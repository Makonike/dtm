...
func handleGrpcBusiness(in *BusiReq, result1 string, busi string) error {
	...
	return status.New(codes.Internal, fmt.Sprintf("unknow resultwwwwwwwwwwwwwwwwwwwwwwwwwww %s", res)).Err()
}
...
func GrpcNewServer() *grpc.Server {
	return grpc.NewServer(grpc.ChainUnaryInterceptor(dtmgimp.GrpcServerLog, middleware.NewJaegerServerMiddleware().UnaryInterceptor))
}
